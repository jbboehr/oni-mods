...
namespace MightyVincent
{
    internal static class LightGridUtil
    {
        ...
        private static IEnumerable<LineBlock> GetLineBlocksOf(Vector2I deltaPoint)
        {
            ...
            if (dx == 0)
            {
                return dy == 0 ? _EMPTY : new[] {dy > 0 ? _DOWN_RL + deltaPoint : _UP_LR + deltaPoint};
            }
            ...
            if (dx > 0)
            {
                if (dy == 0)
                {
                    return new[] {_LEFT_DU + deltaPoint};
                }
